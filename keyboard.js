...
function storedSetup(settings) {
    ...
    if (!settings.hasOwnProperty("marginValue") || settings.marginValue === null) {
		browser.storage.local.set({
			marginValue: 0,
		});
    ...
    if (!settings.hasOwnProperty("fontSize") || settings.fontSize === null) {
		browser.storage.local.set({
			fontSize: 35,
		});
    ...
    fxKeyboard.settings.marginValue = settings.marginValue;
    fxKeyboard.settings.fontSize = settings.fontSize;
...
var fxKeyboard = {
    ...
    _toggleOpen: function (open) {
        ...
        if (open) {
            if (!this.hierarchy.isMaster) {
				...
				kb.style.fontSize = this.settings.fontSize * this.settings.scale + "px";        //ANGEL tamaño de letras
    ...
    //ANGEL Funcion para cambiar estilo de teclas de letras
    ...
    //Funcion cargar estilo de teclas especiales
    ...
    insertKeyboard: function (inputType) {
        ...
        keyb.style.left = "50%";
        keyb.style.marginBottom=this.settings.marginValue+"%";           //ANGEL margin bottom teclado
        keyb.style.bottom = 0;
